...
\author{Shunsuke Kubota\thanks{Correspondence: \texttt{shkub@example.org}.}}
\date{April 2026}
...
\emph{explicit}.  The trusted theorem layer rests on exactly nine axioms,
classified into system-specific and utility categories.
...
\subsection{Trusted vs.\ Untrusted Boundary}
...
\begin{itemize}
  ...
  \item \textbf{Untrusted Infrastructure} (3 files, 8~\sorry{}):
    PDE semigroup bodies (\texttt{Operators.lean}, 6~\sorry{}),
    controlled evolution (\texttt{Control.lean}, 1~\sorry{}), and
    well-posedness uniqueness (\texttt{LocalWellPosedness.lean}, 1~\sorry{}).
\end{itemize}
...
\begin{tikzpicture}[
  ...
  \begin{scope}[on background layer]
    \node[draw=red!60, dashed, thick, rounded corners, fit=(ops)(ctrl)(lwp),
          inner sep=6pt, label={[font=\scriptsize, red!60]below:Untrusted (8 sorry)}] {};
...
Red: untrusted infrastructure (8~\sorry{}).  Gray: examples.
...
\begin{itemize}
  \item The PDE semigroup bodies remain as \sorry{} placeholders
    (8~in infrastructure), awaiting Mathlib's PDE theory to mature.
